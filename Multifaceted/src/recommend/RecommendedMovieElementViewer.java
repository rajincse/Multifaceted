...
import eyerecommend.RecommendationEngine;
import eyerecommend.item.RectangleItem;
import eyetrack.EyeTrackerPivotElementDetector;
import eyetrack.EyeTrackerViewer;


public class RecommendedMovieElementViewer extends Viewer implements JavaAwtRenderer, LayoutViewerInterface , eyerecommend.EyeTrackerViewer{
	...
	private RecommendationEngine et = null;
	...
	public RecommendedMovieElementViewer(String name, IMDBDataSource data) {
		super(name);
		this.data = data;
		...
		et = new RecommendationEngine(this);
	...
	{
		et.block(true);
		RectangleItem[] elementArray = new RectangleItem[layout.getElements().size()];
		for(int i=0;i<elementArray.length;i++)
		{
			PivotLabel label = layout.getElements().get(i).getLabel();
